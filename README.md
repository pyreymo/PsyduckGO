...
经过分别试验了python的`tk`,`pyQt`,`pyglut`等图形库之后，我决定用性能相对较好的`pygame`库编写，也初步实现了弹幕游戏应该具有的功能——开始界面、选关界面、剧情对话、敌人和弹幕的生成、角色的移动闪避、胜负判断等。
<br>
<br>
...
对于并不是主流比例（1920x1080）的屏幕（比如说机房的近方形屏幕 1280x1024），我程序中也做了简单的适配，但是由于时间不足，部分界面可能还是会显示不全。
<br>
## 总结
